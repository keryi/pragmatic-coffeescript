...
scoreMove = (grid, swapCoordinates) ->
  ...
  for word in words when word not in newWords and word not in usedWords
    multiplier++
    moveScore += tileValues[letter] for letter in word
    newWords.push word
  usedWords.concat newWords
...
promptForTile1 = ->
  printGrid()
  console.log 'Please enter the coordinate for tile 1'
  inputCallback = (input) ->
    try
...
promptForTile2 = (x1, y1) ->
  console.log 'Please enter the coordinate for tile 2'
  inputCallback = (input) ->
    try
